...
import SentryBrowserCapturer from "./sentryNode"

test("@sentry/node exception capturer calls Sentry.init on creating", () => {
...
test("@sentry/node exception capturer calls Sentry.captureException", () => {
...
test("@sentry/node exception capturer calls Sentry.captureMessage", () => {
  const capturer = new SentryBrowserCapturer({})
